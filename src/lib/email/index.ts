...
export type ClientDocumentsReadyEmailData = {
  name?: string | null;
  email: string;
  requestId: string;
  amount?: number | string | null;
...
function buildDocumentsReadyEmail(data: ClientDocumentsReadyEmailData): EmailPayload {
  const name = cleanName(data.name);
  const email = normalizeEmail(data.email);
  const requestId = String(data.requestId || "").trim();
  const amount = formatCLP(data.amount);
  ...
  const safeRequestId = escapeHtml(requestId);
  const safeAmount = escapeHtml(amount);
  const safePlate = escapeHtml(plate || "No informada");
  const safeSupportEmail = escapeHtml(SUPPORT_EMAIL);

  const subject = "Tu informe está listo - Prescribe tu Multa";

  const text = [
    `Hola ${name},`,
    "",
    "Tu informe y documentación asociada ya fueron preparados.",
    "",
    `N° de solicitud: ${requestId}`,
    `Patente: ${plate || "No informada"}`,
    `Monto pagado: ${amount}`,
    "",
    "Qué incluye esta entrega:",
    "- Informe de análisis de multas.",
    "- Detalle de multas revisadas.",
    "- Borrador de solicitud de prescripción, cuando corresponda.",
    "- Guía de tramitación personal.",
    "",
    "Importante:",
    "- Revisa también spam, promociones o correo no deseado.",
    `- Si tienes dudas, responde este correo o escríbenos a ${SUPPORT_EMAIL} indicando tu N° de solicitud.`,
    "- Por seguridad, este correo no contiene enlaces directos públicos de descarga.",
    "",
    "Disclaimer legal:",
    "El informe y los documentos entregados son material de apoyo jurídico. No garantizan que el tribunal acoja la solicitud ni que las multas sean eliminadas automáticamente. La decisión final depende del tribunal competente, de los antecedentes del caso y de la tramitación realizada.",
    "",
    "Prescribe tu Multa",
  ].join("\n");
        ...
        <td align="center">
          <table role="presentation" width="100%" cellPadding="0" cellSpacing="0" style="max-width:640px;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e2e8f0;">
            <tr>
              <td style="background:#0f766e;padding:28px 30px;color:#ffffff;">
                <div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;font-weight:700;opacity:.9;">Prescribe tu Multa</div>
                <h1 style="margin:10px 0 0;font-size:26px;line-height:1.25;">Tu informe está listo</h1>
                <p style="margin:10px 0 0;font-size:15px;line-height:1.6;color:#ccfbf1;">Preparamos la documentación asociada a tu solicitud.</p>
              </td>
            </tr>
            <tr>
              <td style="padding:30px;">
                <p style="margin:0 0 18px;font-size:16px;line-height:1.7;">
                  Hola <strong>${safeName}</strong>, tu informe y documentación asociada ya fueron preparados.
                </p>
                ...
                <div style="margin:24px 0;padding:18px;border-radius:14px;background:#ecfdf5;border:1px solid #bbf7d0;">
                  <h2 style="margin:0 0 10px;font-size:17px;color:#065f46;">Qué incluye esta entrega</h2>
                  <ul style="margin:0;padding-left:20px;color:#064e3b;font-size:14px;line-height:1.7;">
                    <li>Informe de análisis de multas.</li>
                    <li>Detalle de multas revisadas.</li>
                    <li>Borrador de solicitud de prescripción, cuando corresponda.</li>
                    <li>Guía de tramitación personal.</li>
                  </ul>
                </div>
                ...
                <div style="margin:24px 0;padding:18px;border-radius:14px;background:#fff7ed;border:1px solid #fed7aa;">
                  <h2 style="margin:0 0 10px;font-size:17px;color:#9a3412;">Importante</h2>
                  <ul style="margin:0;padding-left:20px;color:#7c2d12;font-size:14px;line-height:1.7;">
                    <li>Revisa spam, promociones o correo no deseado.</li>
                    <li>Si tienes dudas, responde este correo o escríbenos a <strong>${safeSupportEmail}</strong> indicando tu N° de solicitud.</li>
                    <li>Por seguridad, este correo no contiene enlaces directos públicos de descarga.</li>
                  </ul>
                </div>
...
export async function sendClientDocumentsReady(
  data: ClientDocumentsReadyEmailData
): Promise<EmailSendResult> {
  const email = normalizeEmail(data.email);
  ...
  if (!data.requestId || String(data.requestId).trim().length === 0) {
    return {
      ok: false,
      provider: "none",
      error: "requestId requerido para correo de documentos listos.",
    };
  }

  return sendEmail(
    buildDocumentsReadyEmail({
      ...data,
      email,
    })
  );
